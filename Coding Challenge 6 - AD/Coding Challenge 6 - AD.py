...
outputDirectory = r"C:\Data\Students_2021\1_Data\Monthly"
if not os.path.exists(outputDirectory):
    os.mkdir(outputDirectory)


for month in listMonths:
    arcpy.env.workspace = os.path.join(r"C:\Data\Students_2021\1_Data", str(month)) # AD Directory path was incorrectly formatted
    listRasters = arcpy.ListRasters("*", "TIF")
...
# Remove any _BQA.tif files that may be in your directory
    listRasters = [x for x in listRasters if "_BQA.tif" not in x] #Not sure why you converted listRasters to a string, this is a failure point.
    print(listRasters)
    arcpy.CompositeBands_management(listRasters,
